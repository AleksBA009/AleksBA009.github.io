...
Статья на русском языке. МИНИМУМ 1200 слов.

СТРОГИЕ ПРАВИЛА:
1. Никаких общих фраз: «кофе — это искусство», «выбор сложен», «надеемся, что помогли». Сразу к делу.
2. Каждый подзаголовок H2 должен содержать от 100 до 200 слов с конкретной информацией.
3. ОБЯЗАТЕЛЬНО используй:
   - Реальные названия моделей, брендов, сортов (для турки — медная турка «Станица», TimA, Гурман; для кофемашин — DeLonghi, Philips, Krups).
   - Цифры: объём в мл, цена в рублях, время в минутах, температура в °C.
   - Сравнения в формате таблицы или списка «модель А vs модель Б».
   - Типичные ошибки новичков и чёткие инструкции, как их избежать.
   - Хотя бы один исторический или технологический факт.
4. ТЕМАТИЧЕСКАЯ ТОЧНОСТЬ: если тема про турку — пиши про турку. Не путай турку с кофемашиной. Для турки используй термины: помол «в пыль», пенка, прогрев в песке, медная турка с оловянным покрытием.
5. FAQ-блок: 3-4 вопроса с развёрнутыми ответами (не менее 3 предложений каждый).
6. Заключение должно содержать ГЛАВНЫЙ ВЫВОД статьи (одно сильное предложение) и практическую рекомендацию.

СТРУКТУРА:
- Введение: начни с парадокса, малоизвестного факта или сильного утверждения, которое зацепит читателя (3-5 предложений).
- 5-7 подзаголовков H2, полностью раскрывающих тему.
- Блок «Часто задаваемые вопросы» (H2).
- Заключение с главным выводом.

Дважды вставь партнёрскую ссылку: <a href="{aff_url}" target="_blank">{aff_text}</a> — органично, там, где это действительно нужно.
Добавь ОДНУ внутреннюю ссылку на другую статью сайта: [{SITE_URL}/...]({SITE_URL}/...) — с логичным URL.

ФОРМАТ ОТВЕТА (строго JSON):
{{"title": "...", "excerpt": "...", "content_markdown": "...", "tags": "кофе, ..."}}

В "content_markdown" — полный Markdown статьи. Заголовки H2 обозначай ## .
Каждый раздел должен быть полным. НЕ ДОПУСКАЮТСЯ разделы из одного предложения.
